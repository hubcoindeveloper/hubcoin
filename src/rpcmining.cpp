...
//    Litecoin and modified for HubCoin.
//
//    Parameters: lookup (int) - How many blocks to look into the past.
//    Returns: Estimated HubCoin network hash rate (uint64_t)
//
...
// WM - Implementation of getnetworkhashps for HubCoin
Value getnetworkhashps(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "getnetworkhashps\n"
            "Returns an estimate of the HubCoin network hash rate.");
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "HubCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "HubCoin is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "HubCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "HubCoin is downloading blocks...");
